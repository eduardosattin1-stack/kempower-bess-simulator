...
# 1. Get the absolute path to the current script's directory
current_dir = Path(__file__).parent if "__file__" in locals() else Path.cwd()

# 2. Define the logo filename (ENSURE THIS MATCHES GITHUB EXACTLY)
LOGO_FILENAME = "kempowerlogo.png" 
...
with st.sidebar:
    if LOGO_PATH.exists():
        ...
        # Diagnostic help for Bruno
        st.error(f"⚠️ Logo not found!")
        st.caption(f"Search Path: {LOGO_PATH}")
        st.caption("Common fix: Check if the filename is 'KempowerLogo.png' instead of 'kempowerlogo.png' on GitHub.")
